...
const TeacherGradesGrid = ({ docs, smaller }) => {
  ...
  const getCombinedAverageGrade = (student) => {
    const firstSessionAverage = getAverageGrade(student.firstSessionMarks);
    const secondSessionAverage = getAverageGrade(student.secondSessionMarks);
    const combinedAverage = (firstSessionAverage + secondSessionAverage) / 2;
    return isNaN(combinedAverage) ? NaN : combinedAverage;
  };
  ...
  return (
    <>
      ...
      >
        ...
        {docs
          ?.sort((a, b) => {
            const avgA = getCombinedAverageGrade(a);
            const avgB = getCombinedAverageGrade(b);
            if (isNaN(avgA) && isNaN(avgB)) return 0;
            else if (isNaN(avgA)) return 1;
            else if (isNaN(avgB)) return -1;
            else return avgB - avgA;
          })
          ?.map((el, i) => {
...
export default TeacherGradesGrid;
