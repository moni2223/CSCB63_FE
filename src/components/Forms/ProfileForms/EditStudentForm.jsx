...
const EditStudentForm = ({ errors, register }) => {
  ...
  return (
    <div className="body-container" style={{ height: "unset" }}>
      <h1 className=" text-md font-bold uppercase">Основна информация <span className="font-medium text-sm italic">(ако запазите промените, ще трябва да влезете в профила си отново)</span></h1>
                ...
                <Inputs.SingleAsyncSelect
                  ...
                  loadOptions={(pattern) =>
                    loadDropdownOptions({
                      ...
                      map: (res) => ({ value: res._id, label: res.name }),
                    })
                  }
                  onChange={(e) => {
                    setValue("workDepartment", e);
                  }}
                />
              </>
            )}
